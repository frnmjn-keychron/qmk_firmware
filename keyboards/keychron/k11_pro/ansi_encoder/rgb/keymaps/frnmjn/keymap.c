...
[NOR] = LAYOUT_69_ansi(
        NO,   NO,     NO,     NO,     NO,     NO,     NO,     NO,     NO,     NO,     NO,      NO,      NO,      NO,      NO,
        NO,   NO,     T_CB,   T_SB,   T_RB,   PGUP,           NO,     RBC,    SBC,    CBC,     NO,      NO,      NO,      NO,      NO,
        NO,   HOME,   NO,     NO,     SFT,    PGDN,           _SX,    _DN,    _UP,    _DX,     END,     NO,      NO,      NO,
        NO,   UNDO,   CUT,    COPY,   PASTE,  REDO,   NO,     NO,     T_QT,   T_DQT,  T_PIPE,  T_PERC,  NO,      NO,
...
[NUM] = LAYOUT_69_ansi(
        ...
        NO,   CMD,    UNDS,   DOT,    KC_0,   NO,             MINS,   KC_4,   KC_5,   KC_6,    PLUS,    NO,      NO,      NO,
...
void autoshift_press_user(uint16_t keycode, bool shifted, keyrecord_t *record) {
    switch (keycode) {
        ...
        case SLSH:
            ...
            break;
        case ENT:
            register_code16((!shifted) ? ENT : ASTR);
            break;
        case MINS:
            if (!shifted)
                register_code16(MINS);
            else {
...
void autoshift_release_user(uint16_t keycode, bool shifted, keyrecord_t *record) {
    switch (keycode) {
        ...
        case SLSH:
            ...
            break;
        case ENT:
            unregister_code16((!shifted) ? ENT : ASTR);
            break;
        case MINS:
            if (!shifted)
                unregister_code16(MINS);
            else {
        ...
        case TAB:
            if (!shifted)
                unregister_code16(TAB);
            else {
                unregister_code16(CMD);
                unregister_code16(SFT);
